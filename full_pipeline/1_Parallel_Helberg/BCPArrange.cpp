...
int main(int argc, char * argv[])
{
    ...
    //vector<int> Pillow {1, 2, 3, 4, 5, 6, 10, 12, 15, 20, 24, 30, 40, 50, 60, 72, 80, 90, 100};
    //for (int threadloop = 0; threadloop < Pillow.size(); threadloop++){
        int threadcount = 5; //Pillow[threadloop];

        time_t now; //for profling speed
        ifstream Input; string inputfile("9.txt"); 
...
//        logname = "/csm_data/experimentalmath/maurer/ParallelLogs/LOG" + N.str();
        logname = "./Logs/T" + TC.str() + "/LogS2L" + N.str();
        ...
        for (r = 0; r < Weight(n+1, 4, d); r++)
        {
            ...
            if (CodeBooks[r].CodeWords.size() > 2) 
            {
                ...
                if (parallel.size() < threadcount)
                {
                    parallel.emplace_back(thread(Biochem, ref(CodeBooks[r].CodeWords), n, ref(CodeBooks[r].GC)));
                }
                else
                {
                    Join(parallel);
        ...
        ofstream Output; string outputfile("out9.txt");
